...
const AdminNavbar = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 lg:left-72 w-full lg:w-[calc(100%-18rem)] h-16 sm:h-20 border-b border-border/40 bg-surface/80 backdrop-blur-md z-[120] px-6 md:px-10 flex items-center justify-between">
            {/* 1. LEFT: LOGO & STATUS */}
            <div className="flex items-center gap-8">
                <Link to="/admin/dashboard" className="flex lg:hidden items-center group gap-4">
                    <div className="size-14 rounded-full border border-border bg-surface-hover/50 flex items-center justify-center overflow-hidden shadow-inner group transition-all hover:border-primary/30">
                        <img 
                ...
                {/* 2. SEARCH ARCHIVE */}
                <div className="hidden lg:flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-surface-hover/50 border border-border focus-within:border-primary/40 focus-within:bg-surface transition-all w-96 group">
                    <Search className="size-4 text-text-secondary  group-focus-within:opacity-100 group-focus-within:text-primary transition-all" />
                    <input 
                        type="text" 
                        placeholder="Search experiences, users, companies..." 
                        className="bg-transparent border-none outline-none text-xs font-bold text-content placeholder:text-text-secondary/40 w-full"
                    />
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                                        ...
                                        {quickNotifications.map((notif) => (
                                            <div key={notif.id} className="p-5 hover:bg-surface-hover/50 transition-colors border-b border-border/10 flex items-start gap-4 cursor-pointer group">
                                                <div className={`size-9 rounded-xl flex items-center justify-center shrink-0 border ${
                                                    notif.type === 'alert' ? 'bg-danger/10 border-danger/20 text-danger' :
                                                    notif.type === 'approved' ? 'bg-success/10 border-success/20 text-success' :
                                                ...
                                                <div className="flex flex-col gap-0.5">
                                                    <div className="flex items-center justify-between gap-2">
                                                        ...
                                                        <span className="text-[8px] font-bold text-text-muted ">{notif.time}</span>
                                                    </div>
                ...
                <div className="relative">
                    <button 
                        ...
                        className="flex items-center gap-3 p-1.5 pr-4 rounded-2xl bg-surface-hover border border-border hover:border-primary/40 transition-all group shadow-sm  from-surface-hover to-surface"
                    >
                        <div className="size-9 rounded-xl  from-primary to-accent p-0.5 shadow-sm overflow-hidden">
                            <div className="size-full rounded-[10px] bg-background flex items-center justify-center overflow-hidden">
                                {user?.profilePicture ? (
                                    <img src={user.profilePicture} alt="Admin Avatar" className="size-full object-cover" />
                                ) : (
                        ...
                        <>
                            ...
                            >
                                ...
                                <button 
                                    ...
                                    className="w-full flex items-center gap-3 px-5 py-3 hover:bg-danger/5 transition-colors group text-danger"
                                >
