    ...
    <div className="relative w-full" ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-2.5 overflow-hidden">
          {Icon && <Icon className={`size-4 shrink-0 transition-colors ${isOpen ? 'text-primary' : 'text-text-muted  group-hover:text-primary/60'}`} />}
          <span className={`text-[13px] font-bold truncate ${!displayValue ? 'text-text-muted ' : 'text-content'}`}>
            ...
            <div className="max-h-[180px] overflow-y-auto custom-scrollbar p-0.5">
              {options.length === 0 ? (
                <div className="py-4 px-3 text-center">
                  <p className="text-[8px] font-black uppercase tracking-widest text-text-muted ">No options found</p>
                </div>
              ) : (
                  ...
                  return (
                    <motion.div
                      ...
                      className={`flex items-center justify-between px-3 py-1.5 rounded-lg cursor-pointer transition-all mb-0.5 last:mb-0
                        ${isSelected ? 'bg-primary text-primary-text shadow-sm shadow-primary/10' : 'hover:bg-primary/10 text-content/80 hover:text-primary'}
                      `}
                    >
