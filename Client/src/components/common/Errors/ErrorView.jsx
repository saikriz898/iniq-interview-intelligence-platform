...
const ErrorView = ({ 
  ...
  const getVisualIcon = () => {
    switch (visualType) {
      ...
      case "denied": return <ShieldAlert className="size-16 text-danger/60" />;
      ...
      case "session": return <LogOut className="size-16 text-danger/60" />;
      case "unavailable": return <EyeOff className="size-16 text-primary/60" />;
      case "validation": return <AlertTriangle className="size-16 text-danger/60" />;
    ...
    >
      ...
      <div className="relative shrink-0 hidden md:block">
        <motion.div 
          ...
          className="w-[340px] h-[360px] rounded-[2.5rem] bg-surface-hover border border-theme flex flex-col items-center justify-center gap-8 p-10 shadow-[0_20px_50px_rgba(0,0,0,0.4)] relative overflow-hidden perspective-1000"
        >
          ...
          <div className="flex flex-col items-center text-center gap-3 relative z-10">
            ...
            <div className={`h-1 w-16 rounded-full ${visualType === 'denied' || visualType === 'session' ? 'bg-danger' : 'bg-primary'} opacity-20`} />
          </div>

          {/* Abstract Data Lines */}
          <div className="w-full flex flex-col gap-3 relative z-10 px-4">
             <div className="h-2 w-full rounded-full bg-card-bg" />
             <div className="h-2 w-2/3 rounded-full bg-card-bg" />
          </div>
        </motion.div>
      ...
      <div className="flex flex-col items-center md:items-start text-center md:text-left gap-8 lg:max-w-xl z-10">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 w-full sm:w-auto pt-4">
          <button 
            ...
            className="w-full sm:w-auto px-10 py-4.5 rounded-2xl bg-primary text-primary-text font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-primary/30 flex items-center justify-center gap-3 group"
          >
            {primaryAction.label}
