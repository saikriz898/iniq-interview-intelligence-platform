      ...
      <div className="w-full max-w-[1400px] mx-auto flex flex-col relative z-10">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-20 mb-12">
          ...
          <div className="flex flex-col items-start">
            ...
            <div className="flex flex-col gap-4 w-full pt-6 border-t border-[var(--border)] mt-2">
              ...
              <div className="flex items-center gap-4">
                {[
                  { Icon: LinkedinIcon, color: "hover:text-primary-text hover:bg-[#0077B5]" },
                  { Icon: XIcon, color: "hover:text-primary-text hover:bg-primary-bg" },
                  { Icon: InstagramIcon, color: "hover:text-primary-text hover:bg-[#E4405F]" },
                  { Icon: YoutubeIcon, color: "hover:text-primary-text hover:bg-[#FF0000]" }
                ].map((item, i) => (
                  <a 
                    key={i} 
                    href="#" 
          ...
          <div className="flex flex-col">
            <h4 className="text-[12px] font-bold uppercase tracking-[0.25em] text-content opacity-70 mb-8 font-['Space_Grotesk']">Quick Links</h4>
            <ul className="flex flex-col gap-5 text-sm font-medium text-text-secondary">
          ...
          <div className="flex flex-col">
            ...
            <ul className="flex flex-col gap-5 text-sm font-semibold text-[var(--text-secondary)]">
              <li className="group"><Link to="/help-center" onClick={() => setIsLoading(true)} className="hover:text-accent transition-all flex items-center gap-2 group-hover:translate-x-1"><Mail className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all" /> Help Center</Link></li>
              <li className="group"><Link to="/contact" onClick={() => setIsLoading(true)} className="hover:text-accent transition-all flex items-center gap-2 group-hover:translate-x-1"><Mail className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all" /> Contact</Link></li>
              <li className="group"><Link to="/privacy-policy" onClick={() => setIsLoading(true)} className="hover:text-accent transition-all flex items-center gap-2 group-hover:translate-x-1"><Shield className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all" /> Privacy Policy</Link></li>
              <li className="group"><Link to="/terms-of-service" onClick={() => setIsLoading(true)} className="hover:text-accent transition-all flex items-center gap-2 group-hover:translate-x-1"><FileText className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all" /> Terms of Service</Link></li>
              <li className="group"><Link to="/cookie-policy" onClick={() => setIsLoading(true)} className="hover:text-accent transition-all flex items-center gap-2 group-hover:translate-x-1"><FileText className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-all" /> Cookie Policy</Link></li>
            </ul>
          </div>
        </div>

        {/* Newsletter Section */}
        <div className="w-full p-10 md:p-14 mb-16 card-base glow-border relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10">
          <div className="flex flex-col lg:flex-1 text-center lg:text-left relative z-10">
            <h3 className="text-3xl md:text-4xl font-bold font-['Space_Grotesk'] tracking-tight text-content mb-4 uppercase">Stay Synchronized</h3>
            <p className="text-sm md:text-base font-medium text-text-secondary max-w-lg mx-auto lg:mx-0 leading-relaxed ">
              Real-time interview nodes and preparation protocols delivered to your terminal.
            </p>
          </div>
          <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto lg:min-w-[480px] relative z-10">
            <div className="relative w-full group">
              <Mail className="absolute left-5 top-1/2 -translate-y-1/2 size-4 text-text-secondary  group-focus-within:text-primary transition-all" />
              <input 
                type="email" 
                value={email}
                ...
                className="w-full h-14 pl-12 pr-6 bg-surface border border-border rounded-2xl text-sm font-medium focus:outline-none focus:border-primary/40 transition-all text-content placeholder:text-text-secondary/30 disabled: shadow-inner"
                required
              />
            </div>
            <button 
              ...
              className={`btn-primary h-14 px-10 text-xs tracking-widest uppercase shadow-xl ${status === 'success' ? 'bg-success' : ''}`}
            >
              {status === "loading" ? (
                <div className="size-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
              ) : status === "success" ? (
                <><CheckCircle2 className="size-4" /> Sync Active</>
              ) : (
                "Initialize Sync"
              )}
            </button>
          </form>
          <div className="absolute top-0 right-0 w-1/3 h-full  from-primary/5  pointer-events-none" />
        </div>

        {/* Bottom Bar */}
        <div className="w-full pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.4em] text-text-secondary  font-['Space_Grotesk']">
            © 2026 INIQ // OPEN_SOURCE_INTELLIGENCE
          </p>
          <div className="flex items-center gap-6">
            <p className="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.4em] text-text-secondary  font-['Space_Grotesk']">
              NODE_STABILITY: OPTIMAL
            </p>
            <div className="size-1.5 rounded-full bg-success animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
          </div>
        </div>
      </div>
    </footer>
