...
const InternalAppShell = ({ children, title = "Platform", breadcrumb = "Home" }) => {
  ...
  const SidebarContent = (
    <div className="flex flex-col h-full bg-surface border-r border-border py-8 px-4">
      {/* Brand Identity */}
      <div className="flex items-center px-4 mb-12 group">
      ...
      <div className="flex flex-col gap-2 flex-grow">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-text-muted  px-4 mb-2">Main Menu</span>
        {navItems.map((item) => {
      ...
      <div className="mt-auto px-2">
        <div className="p-4 rounded-2xl bg-surface-hover border border-border flex items-center gap-3">
          <div className="size-10 rounded-full  from-primary to-accent p-0.5 shadow-md">
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[9px] font-bold text-text-muted uppercase tracking-widest ">Candidate</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="h-screen w-full flex bg-background overflow-hidden selection:bg-primary selection:text-primary-text">
      {/* 1. Desktop Sidebar */}
        ...
        {isSidebarOpen && (
          <>
            <motion.div 
              ...
              className="fixed inset-0 bg-secondary-bg backdrop-blur-sm z-[90] lg:hidden"
            />
            <motion.div
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
        ...
        <header className="h-[80px] shrink-0 border-b border-border px-6 md:px-10 flex items-center justify-between bg-surface/50 backdrop-blur-md z-50">
          ...
          <div className="flex items-center gap-3">
             <div className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-hover border border-border group focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                <Search className="size-4 text-text-muted  group-focus-within:opacity-100" />
                <input 
                  type="text" 
             ...
             <button className="p-2.5 rounded-xl bg-surface-hover border border-border text-text-muted hover:text-primary transition-all relative">
                ...
                <span className="absolute top-2 right-2 size-2 bg-danger rounded-full border-2 border-surface" />
             </button>
          </div>
        </header>
