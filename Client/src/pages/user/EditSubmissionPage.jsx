...
const EditSubmissionPage = () => {
  ...
  const [submissionMeta, setSubmissionMeta] = useState({ status: '', adminFeedback: '' });
  const totalSteps = 5;
  ...
  const [formData, setFormData] = useState({
    ...
    questions: [
      { id: Date.now(), text: '', codeSnippet: '', output: '', explanation: '', topic: 'Data Structures', roundId: '' }
    ],
    difficulty: 'Medium',
    verdict: 'Pending',
    advice: ''
  ...
  const fetchExperienceDetails = async () => {
      try {
          ...
          if (data.status === 'Approved') {
              toast.error('Approved submissions cannot be edited.');
              navigate('/my-submissions');
              return;
          }
          setSubmissionMeta({ status: data.status, adminFeedback: data.adminFeedback });
          setFormData({
              ...
              salary: data.salary || '',
              interviewMode: data.interviewMode || 'Online',
              applicationMethod: data.applicationMethod || 'Off-campus',
              summary: data.summary || '',
              processOverview: data.processOverview || '',
              topics: data.topics?.length ? data.topics.map(t => ({ ...t, id: t._id || Date.now() + Math.random() })) : [{ id: Date.now(), name: 'Data Structures & Algorithms', details: '' }],
              rounds: data.rounds?.length ? data.rounds.map(r => ({ ...r, id: r._id || Date.now() + Math.random(), title: r.title, questions: r.desc, explanation: r.details })) : [{ id: Date.now(), title: 'Round 1: Initial Technical Screening', questions: '', solution: '', explanation: '', videoLink: '' }],
              questions: data.questions?.length ? data.questions.map(q => ({ ...q, id: q._id || Date.now() + Math.random(), roundId: q.roundId || '', output: q.output || '' })) : [{ id: Date.now(), text: '', codeSnippet: '', output: '', explanation: '', topic: 'Data Structures', roundId: '' }],
  ...
  return (
    <UserAppShell theme={theme} toggleTheme={toggleTheme} isLoading={isLoading} noPadding={true}>
      <CustomToaster />
      <AnimatePresence>
        {showSuccess && <SuccessAnimation setShowSuccess={setShowSuccess} setCurrentStep={setCurrentStep} navigate={navigate} newSubmissionId={newSubmissionId} />}
      </AnimatePresence>
      ...
      <div className="h-full w-full flex flex-col md:flex-row overflow-hidden bg-background/50 pb-16 lg:pb-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-background/30 relative overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-hidden relative z-10 flex flex-col">
                {submissionMeta.status && submissionMeta.status !== 'Pending Review' && submissionMeta.status !== 'Draft' && (
                    <div className={`shrink-0 mx-4 md:mx-6 mt-4 p-4 rounded-xl border flex items-start gap-3 shadow-sm ${
                        submissionMeta.status === 'Rejected' 
                            ? 'bg-danger/10 border-danger/20 text-danger' 
                            : 'bg-success/10 border-success/20 text-success'
                    }`}>
                        {submissionMeta.status === 'Rejected' ? <AlertCircle className="size-5 shrink-0 mt-0.5" /> : <CheckCircle2 className="size-5 shrink-0 mt-0.5" />}
                        <div className="flex flex-col gap-1">
                            <span className="text-sm font-black uppercase tracking-widest leading-none">{submissionMeta.status}</span>
                            {submissionMeta.adminFeedback && (
                                <p className="text-sm font-medium opacity-90 mt-1">{submissionMeta.adminFeedback}</p>
                            )}
                            {submissionMeta.status === 'Rejected' && (
                                <span className="text-xs font-bold mt-1 opacity-75 uppercase tracking-wider">Please update your submission and submit again to resend for review.</span>
                            )}
                        </div>
                    </div>
                )}
                            ...
                            {currentStep === 5 && (
                                <div className="space-y-5">
                                     ...
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-text-muted  ml-1">Final Verdict</label>
                                            <CustomSelect 
                                                value={formData.verdict}
                                                onChange={(val) => setFormData({...formData, verdict: val})}
                                                options={['Selected', 'Rejected', 'Pending']}
                                                icon={CheckCircle2}
                                            />
                                        </div>
                                    </div>
...
// UI UTILS
const SuccessAnimation = ({ setShowSuccess, setCurrentStep, navigate, newSubmissionId }) => {
                        ...
                        {phase !== 'deploying' ? (
                            ...
                            >
                                <motion.div 
                                    ...
                                    className="size-28 rounded-[2.5rem]  from-primary to-accent flex items-center justify-center shadow-[0_20px_50px_rgba(99,102,241,0.4)] mb-8 border-2 border-theme relative"
                                >
                                    <Rocket className="size-12 text-primary-text drop-shadow-lg" />
                                    <motion.div 
                                ...
                                <h2 className="text-2xl font-bold text-content uppercase tracking-tight leading-none mb-3 font-['Space_Grotesk']">Submission Complete</h2>
                                <div className="py-2 px-6 rounded-full bg-success/10 border border-success/20 flex flex-col items-center gap-2">
                                    <span className="text-[9px] font-bold text-success uppercase tracking-[0.3em] font-['Space_Grotesk']">Successfully Uploaded</span>
                                </div>
                                <p className="text-[10px] font-medium text-text-muted uppercase tracking-widest mt-4">
                                ...
                                <div className="flex items-center gap-4 mt-8 w-full max-w-sm">
                                    <button
                                        ...
                                        className="flex-1 py-3 px-6 rounded-xl border border-border/40 hover:bg-surface-hover hover:border-primary/30 transition-all font-bold text-xs uppercase tracking-widest text-text-muted hover:text-content"
                                    >
                                        Dashboard
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 py-3 px-6 rounded-xl bg-primary hover:bg-primary-hover shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all font-bold text-xs uppercase tracking-widest text-primary-text flex items-center justify-center gap-2"
                                    >
                                        View Details
                                        <ChevronRight className="size-4" />
                                    </button>
