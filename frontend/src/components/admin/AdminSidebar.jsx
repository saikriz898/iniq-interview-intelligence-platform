  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { label: 'Manage Roles', path: '/admin/roles', icon: ShieldCheck },
      ]
    }
  ];
    ...
    <aside className="fixed top-16 sm:top-20 left-0 bottom-0 w-64 md:w-72 border-r border-border/40 bg-surface/80 backdrop-blur-md z-[100] hidden lg:flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-8 px-5 pb-10">
        {navGroups.map((group, idx) => (
          <div key={idx} className="flex flex-col gap-2 mb-10 shrink-0">
            <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-text-secondary opacity-40 px-4 mb-3 font-['Space_Grotesk']">{group.group}</span>
                ...
                <Link
                  ...
                  className={`flex items-center gap-3.5 px-4 py-3 rounded-xl transition-all group relative ${
                    isActive 
                      ...
                      : 'text-text-secondary hover:bg-surface-hover hover:text-content border border-transparent'
                  }`}
                >
                  <item.icon className={`size-4.5 transition-transform group-hover:scale-110 ${isActive ? 'text-primary' : 'opacity-40'}`} />
                  <span className="text-sm font-bold tracking-tight font-['Space_Grotesk']">{item.label}</span>
                  ...
                  {isActive && (
                    <motion.div layoutId="activeAdminNav" className="size-1.5 rounded-full bg-primary ml-auto shadow-sm shadow-primary/40" />
                  )}
      ...
      <div className="p-8 px-5 border-t border-border bg-background/20 backdrop-blur-sm shrink-0">
        <button 
          ...
          className="w-full flex items-center gap-3.5 px-4 py-3 rounded-2xl text-text-secondary hover:text-red-500 hover:bg-red-500/5 transition-all group border border-transparent hover:border-red-500/20 shadow-sm"
        >
          <div className="size-10 rounded-xl bg-red-500/10 border border-red-500/10 flex items-center justify-center text-red-500 group-hover:bg-red-500 group-hover:text-white transition-all shadow-inner">
             <LogOut className="size-5 transition-transform group-hover:-translate-x-1" />
          </div>
          <span className="text-[10px] font-bold uppercase tracking-widest font-['Space_Grotesk']">Logout Node</span>
        </button>
      </div>
