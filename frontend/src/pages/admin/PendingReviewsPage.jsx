...
const PendingReviewsPage = () => {
    ...
    const fetchSubmissions = async () => {
        try {
            ...
            if (Array.isArray(data)) {
                // Keep only pending
                const pending = data.filter(exp => exp.status === 'Pending Review').map(exp => ({
                    id: exp._id,
                    company: exp.company,
                    role: exp.role,
    ...
    const handleAction = async (id, status) => {
        try {
            ...
            if (res.ok) {
                toast.success(`Successfully ${status.toLowerCase()} experience`);
                fetchSubmissions();
            } else {
                toast.error(`Failed to ${status.toLowerCase()}`);
            }
        } catch (err) {
            console.error(err);
                                    ...
                                    <div className="p-6 rounded-2xl bg-surface border border-border hover:border-primary/20 hover:shadow-xl hover:shadow-black/[0.02] transition-all flex flex-col gap-6 h-full relative overflow-hidden bg-gradient-to-br from-surface to-surface-hover/20">
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-border/40">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[8px] font-bold text-text-muted opacity-40 uppercase tracking-widest mt-1.5">{item.submittedDate}</span>
                                                </div>
                                            </div>
                                            <button 
                                        ...
                                        <div className="grid grid-cols-2 gap-2 mt-2">
                                            <button 
                                                ...
                                                className="flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-500 text-white text-[9px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/10 hover:translate-y-[-1px] active:scale-95 transition-all">
                                                <CheckCircle2 className="size-3.5" />
                                                Approve
                                            </button>
                                            <button 
                                                onClick={() => handleAction(item.id, 'Rejected')}
                                                className="flex items-center justify-center gap-2 py-3 rounded-xl bg-rose-500 text-white text-[9px] font-black uppercase tracking-widest shadow-lg shadow-rose-500/10 hover:translate-y-[-1px] active:scale-95 transition-all">
