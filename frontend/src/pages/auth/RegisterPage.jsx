...
const RegisterPage = () => {
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case 1: return <Step2Password nextStep={nextStep} prevStep={prevStep} formData={formData} updateFormData={updateFormData} showPassword={showPassword} setShowPassword={setShowPassword} />;
      case 2: return <Step1Name nextStep={nextStep} prevStep={prevStep} formData={formData} updateFormData={updateFormData} />;
  ...
  const renderLeftVisual = () => {
    switch (step) {
      ...
      case 1: return <VisualSecurity />;
      case 2: return <VisualNameCard formData={formData} />;
                    ...
                    {[0, 1, 2, 3, 4, 5].map((s) => (
                      <div 
                        ...
                        className={`h-1 rounded-full transition-all duration-500 ${
                          step >= s ? "w-4 bg-primary" : "w-1.5 bg-border/40"
                        }`} 
                      />
                    ))}
                  </div>
...
const Step1Name = ({ nextStep, prevStep, formData, updateFormData }) => (
  <div className="flex flex-col gap-6">
    ...
    <div className="mt-2 flex gap-3">
      <button onClick={nextStep} className="w-1/3 py-4 text-xs font-bold text-text-muted bg-surface-hover hover:bg-border/30 rounded-xl transition-all border border-border">Skip</button>
      <button onClick={nextStep} className="btn-primary flex-1 py-4 text-xs tracking-[0.2em] font-black shadow-lg">Continue</button>
    </div>
  </div>
);
