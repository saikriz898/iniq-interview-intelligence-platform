            ...
            {pageLoading && (
                <div className="absolute inset-0 z-[100] bg-background/60 backdrop-blur-sm flex items-center justify-center">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary animate-pulse">Syncing Intelligence...</span>
                    </div>
                </div>
            )}
                ...
                <div className="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-border/40 lg:border-l-2 lg:border-l-primary/5 bg-surface/5 flex flex-col p-8 md:p-10 shrink-0 relative z-20 overflow-y-auto no-scrollbar">
                    ...
                    <div className="flex flex-col items-center text-center gap-6 mb-12">
                        <div className="relative group/avatar">
                            ...
                            <Link to="/profile/edit" className="absolute bottom-1 right-1 size-11 rounded-2xl bg-surface border border-border shadow-2xl flex items-center justify-center text-text-muted hover:text-primary hover:border-primary transition-all z-20 active:scale-90 group/cam">
                        ...
                        <div className="flex flex-col gap-3 mt-4">
                            ...
                            <div className="px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-[10px] font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2 self-center shadow-sm">
                                ...
                                {profileData.role === 'admin' ? 'Supreme Overseer' : 'Elite Vanguard'}
                            </div>
                        </div>

                        <div className="w-full flex flex-col gap-3 py-6 border-y border-border/40 mt-2">
                             <IdentityDetail icon={Mail} value={profileData.email} />
                             <IdentityDetail icon={MapPin} value={profileData.location || 'Location Not Set'} />
                             <IdentityDetail icon={Globe} value={profileData.domain || 'Domain Not Set'} />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                        ...
                        <div className="flex flex-col gap-2">
                            <SidebarSocialLink icon={ExternalLink} label="Portfolio" value={profileData.portfolio} color="hover:text-primary" />
                            <SidebarSocialLink icon={FileText} label="Resume Dossier" value={profileData.resume} color="hover:text-emerald-500" />
                            <SidebarSocialLink icon={Linkedin} label="LinkedIn" value={profileData.linkedin} color="hover:text-blue-500" />
                            <SidebarSocialLink icon={Github} label="GitHub" value={profileData.github} color="hover:text-content" />
                        </div>
                    </div>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-5 gap-8">
                                ...
                                <div className="xl:col-span-3 p-8 md:p-10 rounded-[2.5rem] bg-surface/80 backdrop-blur-xl border border-border/20 shadow-2xl flex flex-col gap-8 relative overflow-hidden h-fit">
                                    ...
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <p className="text-xl font-bold text-content leading-relaxed italic font-['Sora'] opacity-80 mt-2">
                                            "{profileData.bio || 'Synthesizing complex interview patterns into actionable intelligence. Focused on high-performance distributed systems and architecture.'}"
                                        </p>
                                    </div>

                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-8 border-t border-border/40">
                                        <DossierInfo label="Current Clearance" value={profileData.role === 'admin' ? 'Level 5 (Admin)' : 'Level 4 (Elite)'} icon={ShieldCheck} />
                                        <DossierInfo label="Specialization" value={profileData.domain || 'Not Specified'} icon={Zap} />
                                    </div>
                                ...
                                <div className="xl:col-span-2 p-8 md:p-10 rounded-[2.5rem] bg-surface/40 backdrop-blur-xl border border-border/20 shadow-xl flex flex-col gap-8 relative overflow-hidden h-fit">
                                    ...
                                    <div className="p-8 rounded-[2rem] bg-background/50 border border-border/20 flex flex-col items-center gap-6 group hover:border-emerald-500/20 transition-all border-dashed mt-2">
                                        ...
                                        <a href={profileData.resume || '#'} target="_blank" rel="noreferrer" className="w-full py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-black text-emerald-500 uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all shadow-sm text-center">
                                            {profileData.resume ? 'View Asset' : 'No Asset Uploaded'}
                                        </a>
                                    </div>
...
const IdentityDetail = ({ icon: Icon, value }) => (
    <div className="flex items-center gap-4 text-xs font-bold text-text-muted/90 group/detail cursor-default">
        ...
        <span className="truncate group-hover/detail:text-content transition-colors">{value || 'Not Set'}</span>
    </div>
);
...
const SidebarSocialLink = ({ icon: Icon, label, value, color }) => (
    <a href={value ? (value.startsWith('http') ? value : `https://${value}`) : '#'} target="_blank" rel="noreferrer" className={`p-3 rounded-xl bg-surface-hover/20 border border-border/20 group hover:border-primary/30 transition-all cursor-pointer flex items-center justify-between ${color} ${!value ? 'opacity-30 cursor-not-allowed' : ''}`}>
        <div className="flex items-center gap-3">
