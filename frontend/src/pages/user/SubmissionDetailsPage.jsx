...
const SubmissionDetailsPage = () => {
  ...
  if (!submission) return <div className="h-screen flex items-center justify-center font-black uppercase tracking-widest text-text-muted">Loading Journey...</div>;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
            <div className="flex items-start gap-5">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-4 text-xs font-bold text-text-muted opacity-60">
                        <span className="flex items-center gap-1.5"><Calendar className="size-3.5" /> Submitted {new Date(submission.createdAt).toLocaleDateString()}</span>
                        <span className="flex items-center gap-1.5 uppercase tracking-widest"><Briefcase className="size-3.5" /> {submission.candidateExperience} Years Experience</span>
                    </div>
                </div>
            </div>
            ...
            <div className={`px-6 py-3 rounded-2xl flex items-center gap-3 border shadow-sm ${statusStyle(submission.status)}`}>
                ...
                <div className="flex flex-col leading-none">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest mt-1">{submission.status}</span>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
            ...
            <div className="lg:col-span-2 flex flex-col gap-10">
                ...
                <Section title="Interview Process Overview" icon={FileText}>
                    <p className="text-sm font-bold text-text-muted leading-[1.8] opacity-80 whitespace-pre-wrap">
                            ...
                            >
                                ...
                                <div className="space-y-8">
                                    <div className="flex flex-col gap-3">
                                        ...
                                        <p className="p-5 rounded-2xl bg-surface-hover border border-border text-sm font-bold text-content leading-relaxed whitespace-pre-wrap">
                                            {round.questions}
                                        </p>
                                    </div>
                                    ...
                                    {round.explanation && (
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <p className="text-sm font-bold text-text-muted leading-[1.8] opacity-70 whitespace-pre-wrap">
                                                {round.explanation}
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    {round.videoLink && (
                                        <a href={round.videoLink} target="_blank" rel="noopener noreferrer" className="w-fit flex items-center gap-2.5 px-5 py-3 rounded-xl bg-primary/5 border border-primary/10 text-xs font-black uppercase tracking-widest text-primary hover:bg-primary hover:text-white transition-all shadow-sm">
                ...
                <div className="p-8 rounded-[2.5rem] bg-surface border border-border shadow-xl shadow-black/5 flex flex-col gap-6 sticky top-28">
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col gap-2">
                            ...
                            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-[11px] font-black uppercase tracking-widest w-fit shadow-sm ${submission.verdict === 'Selected' ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
                                {submission.verdict === 'Selected' ? <CheckCircle2 className="size-3.5" /> : <XCircle className="size-3.5" />}
                                {submission.verdict}
                            </div>
                        </div>
