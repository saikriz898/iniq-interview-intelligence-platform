...
const SubmitExperiencePage = () => {
  ...
  const [rounds, setRounds] = useState([
    { id: 1, title: 'Round 1: Initial Technical Screening', questions: '', solution: '', explanation: '', videoLink: '' }
  ]);

  const addRound = () => {
    const newRound = {
      id: Date.now(),
      title: `Round ${rounds.length + 1}: Technical`,
      questions: '',
      solution: '',
      explanation: '',
      videoLink: ''
    ...
    <UserAppShell theme={theme} toggleTheme={toggleTheme} isLoading={isLoading} noPadding={true}>
      ...
      <div className="h-full w-full flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-surface/[0.02] relative overflow-hidden">
                    ...
                    >
                        <div className="max-w-4xl mx-auto w-full flex flex-col gap-6 h-full">
                            {currentStep === 1 && (
                                <div className="space-y-10">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <FormField label="Company Intelligence" placeholder="e.g. Google India" icon={Building2} />
                                        <FormField label="Targeted Role" placeholder="e.g. Senior Software Engineer" icon={Briefcase} />
                                    </div>
                                    <div className="flex flex-col gap-3">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-text-muted opacity-80 ml-1">The Preparation Narrative</label>
                                        <textarea 
                                            ...
                                            placeholder="Chronicle your prep strategy, key resources, and how the opportunity surfaced..."
                                        ></textarea>
                                    </div>
                                </div>
                            )}
                            ...
                            {currentStep === 2 && (
                                <div className="space-y-10">
                                    <div className="flex flex-col gap-3">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-text-muted opacity-80 ml-1">Workflow Synopsis</label>
                                        <textarea 
                                            ...
                                            placeholder="Outline the sequence of events from recruiter call to final onsite..."
                                        ></textarea>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <FormField label="DSA Arsenal" placeholder="DP, Graphs, Trees" icon={Code} />
                                        <FormField label="Scale Focus (HLD)" placeholder="Caching, Partitioning" icon={Layout} />
                                        <FormField label="Logic Focus (LLD)" placeholder="Vending Machine, LL" icon={AlertCircle} />
                                    </div>
                                </div>
                            )}
                            ...
                            {currentStep === 3 && (
                                <div className="h-full flex flex-col min-h-0">
                                    <div className="flex flex-col mb-8 shrink-0">
                                        <h3 className="text-lg font-black text-content italic leading-none">Round Architecture</h3>
                                        <p className="text-[9px] font-bold text-text-muted opacity-60 uppercase tracking-widest mt-1">Documenting technical deep-dives</p>
                                    </div>
                                        ...
                                        {rounds.map((round, idx) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between mb-8">
                                                    <span className="px-4 py-1.5 rounded-full bg-primary/10 text-primary text-[9px] font-black uppercase tracking-widest">MODULE R-{idx + 1}</span>
                                                ...
                                                <div className="grid grid-cols-1 gap-8">
                                                    <FormField label="Specialization Title" defaultValue={round.title} icon={Sparkles} />
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                                        <div className="flex flex-col gap-3">
                                                            <label className="text-[10px] font-black uppercase tracking-widest text-text-muted opacity-60 ml-1">Key Interrogations</label>
                                                            <textarea className="w-full min-h-[120px] p-6 rounded-3xl bg-surface/40 border border-border/40 focus:border-primary outline-none transition-all text-xs font-bold text-content resize-none caret-primary" placeholder="List questions and challenges..."></textarea>
                                                        </div>
                                                        <div className="flex flex-col gap-3">
                                                            <label className="text-[10px] font-black uppercase tracking-widest text-text-muted opacity-60 ml-1">Resolution Strategy</label>
                                                            <textarea className="w-full min-h-[120px] p-6 rounded-3xl bg-surface/40 border border-border/40 focus:border-primary outline-none transition-all text-xs font-mono text-content resize-none caret-primary" placeholder="Approach taken or code logic..."></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </motion.div>
                                ...
                                <div className="space-y-8">
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <select className="w-full p-6 rounded-[2rem] bg-surface/40 border border-border/40 focus:border-primary focus:bg-surface/60 outline-none transition-all text-sm font-bold text-content appearance-none cursor-pointer caret-primary shadow-sm hover:border-primary/40">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-text-muted opacity-80 ml-1">Legacy Advice</label>
                                            <textarea 
            ...
            <div className="h-20 border-t border-border/40 px-12 flex items-center justify-between shrink-0 bg-background/20 backdrop-blur-sm relative z-10">
                <button 
